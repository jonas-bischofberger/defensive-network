...
for f in files:
    ...
    for (defending_team, defender_name), df_def in df.groupby(["defending_team", "defender_name"]):
        ...
        # As receiver (C only): x_target_norm / y_target_norm
        df_C = df_inv[df_inv["possessionEvents.passOutcomeType"] == "C"]
        recv_pos = (df_C[["player_name_2", "player_id_2", "x_target_norm", "y_target_norm"]]
                    .dropna(subset=["player_name_2"])
        ...
        # ── node_size: B/D pass count per passer (nan if 0) ─────────────────
        df_BD    = df_inv[df_inv["possessionEvents.passOutcomeType"].isin(["B", "D"])]
        bd_count = (df_BD.groupby("player_name_1")
                    .size()
                    .reset_index(name="node_size")
                    .rename(columns={"player_name_1": "player_name"}))

        # ── Node table ───────────────────────────────────────────────────────
        nodes = pos_agg.merge(bd_count, on="player_name", how="left")
        # starter status (nan if not found)
        ...
        edges = (df_C.dropna(subset=["player_name_2"])
                 .groupby(["player_name_1", "player_id_1", "player_name_2", "player_id_2"])
                 .agg(**agg_dict)
                 .reset_index()
                 .rename(columns={"player_name_1": "passer_name", "player_id_1": "passer_id",
                                  "player_name_2": "receiver_name", "player_id_2": "receiver_id"}))
        for col, val in reversed(list(meta_dict.items())):
            edges.insert(0, col, val)
        all_edges.append(edges)

pd.concat(all_nodes, ignore_index=True).to_csv(
    os.path.join(HERE, "2026-05-05_player_net_m1_nodes.csv"), index=False)
pd.concat(all_edges, ignore_index=True).to_csv(
    os.path.join(HERE, "2026-05-05_player_net_m1_edges.csv"), index=False)
print("Done")
