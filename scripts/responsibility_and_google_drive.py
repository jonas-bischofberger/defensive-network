...
def main():
    ...
    if _do_analysis:
        ...
        for competition in selected_competitions:
            ...
            if st.toggle("FIFA Correlations", True):
                with st.expander("FIFA", True):
                    ...
                    min_minutes = st.number_input("Minimum minutes played by player for FIFA correlations.",
                                                  min_value=0, value=DEFAULT_MINUTES)
                    ...
                    for x_variable in x_variables:
                        ...
                        for y_nr, y_variable in enumerate(y_variables):
                            ...
                            def bootstrapped_correlation_coefficient(x, y, n_bootstraps=1000):
                                ...
                                st.write(
                                    f"Bootstrapped correlation coefficient: {estimate:.3f} (95% CI: [{lower:.3f}, {upper:.3f}])")
                            ...
                            st.write("df_agg_player_only_cb")
                            st.write(df_agg_player_only_cb)
                            st.write(df)
                            assert len(df_agg_player_only_cb) > 0, "No CBs in df_agg_player_only_cb"
                            correlation_coefficient_only_CBs = df_agg_player_only_cb[x_variable].corr(
                                df_agg_player_only_cb[y_variable])
                            spearman_correlation_only_cbs = df_agg_player_only_cb[x_variable].corr(
                                df_agg_player_only_cb[y_variable], method="spearman")
            ...
            st.write("f")
            if st.toggle("Matchsum Descriptives", True, key=f"{competition}_matchsum_descriptives"):
                # Position counts
            ...
            if st.toggle("Season-by-season correlation", True, key=f"{competition}_season_by_season_correlation"):
            ...
            data = []
            if st.toggle("ICC", True, key=f"{competition}_icc"):
            ...
            if st.toggle("Bootstrapped Season-Level ICC", False, key=f"{competition}_bootstrapped_season_level_icc"):
            ...
            if st.toggle("Histograms", True, key=f"{competition}_histograms"):
            ...
            if st.toggle("KPI correlations as heatmap", True, f"{competition}_kpi_heatmap"):
